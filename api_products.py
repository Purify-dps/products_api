...
def calculation(product, working_hours):
    ...
    if True in lists:
        ...
        my_array['co_emissions'] = co_emissions / 100
        my_array['savings_euros'] = my_array['co_emissions'] * 0.0132
...
def recommendationSystem(working_hours):
    ...
    df['co2_emissions'] = (working_hours * 600 * df['Avg cons'])/100
    df['savings_euros'] = 0.0132 * df['co2_emissions'] 
    df['rank'] = df.index + 1
